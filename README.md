...
Instead of relying on a slow, generic LLM-as-a-judge approach, our system utilizes a **Two-Stage Retriever/Re-ranker** architecture—an industry best practice for search infrastructure.

**Stage 1: Fast Heuristic Retrieval (Layers 1-6)**
Processes 100,000 candidates in under 20s to find the top 1,000.
...
**Stage 2: Dense Semantic Re-ranking (Layer 7)**
7. **Semantic Embedding Fallback:** Initializes `sentence-transformers` (`all-MiniLM-L6-v2`) to compute true dense vector similarity between the job description and the profile summaries of the Top 1,000 candidates. This guarantees "semantic, not just syntactic" understanding without sacrificing performance.
